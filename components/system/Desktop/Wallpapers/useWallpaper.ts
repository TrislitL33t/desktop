...
const useWallpaper = (
  ...
  const getAllImages = useCallback(
    async (baseDirectory: string): Promise<string[]> => {
      try {
        ...
        return await entries.reduce<Promise<string[]>>(
          async (imagesPromise, entry) => {
            ...
            try {
              ...
              if (stat.isDirectory()) {
                const nestedImages = await getAllImages(entryPath);
  ...
  const loadWallpaper = useCallback(
    async (keepCanvas?: boolean) => {
      if (!desktopRef.current) return; // Check if the desktop reference is null
      resetWallpaper();
      ...
      if (
        ...
        const wallpaperModule =
          await WALLPAPER_PATHS[wallpaperName]().catch(fallbackWallpaper);

        if (wallpaperModule?.default) {
          // Access the default export
          const wallpaperFunc = wallpaperModule.default;
          await wallpaperFunc(desktopRef.current, config, fallbackWallpaper);
        }
      } else {
        setWallpaper("VANTA");
      }
